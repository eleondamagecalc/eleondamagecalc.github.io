...
let swarm1 = document.getElementById("swarm1");
let swarm2 = document.getElementById("swarm2");
...
function getMultiplier(loom1, loom2, move, movePower, crit, ult, repeat, hits, swarm, snowball, hitConfirmer = false, level, ul = false, second = false, detailed = false, withoutSlapDown = true, takeSecondaryType = false, foulHit = false) {
   ...
   if (ability2 != "Spectrum Plate" && ability1 == "Power Buster") {
    ...
    else if (ability1 == "Overshadow" && tempType == "	Basic") {
        tempType = "Dark";
        multi *= 1.2;
        stuffUsed.ability1 = ability1;
    }
...
if (rocky.checked && !(loom2.types.includes("Geo")) && !(loom2.types.includes("Metal")) && !(ability == "Mountaineer" || ability == "Rockfall" || ability == "Extremophile")) {
	...
	/*if ((itemB.includes("burst") && move.knockOff) || (itemB != "None" && move.knockOff == true && (withoutKnockOff || ability2 == "Clingy"))) {
        if (move.name == "Knock Off") multi *= 1.5;
        stuffUsed.item2 = itemB;
        stuffUsed.ability2 = (ability2 == "Clingy" ? ability2 : stuffUsed.ability2);
    }
    else if (ability1 == "Sly" && move.knockOff == true) {
        tempItem = (second == false ? item2.value : item1.value);
        if (tempItem != "None" && withoutKnockOff) {
            multi *= 1.5;
            stuffUsed.item2 = tempItem;
        }
    }*/
