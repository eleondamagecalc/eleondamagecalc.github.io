...
function update(updatePower = false, updateBaseStats = false) {
    ...
    updateDiseasedColors();
}

function updateStageColors() {
    let stageSelects = [
        atkStages1, defStages1, atkRStages1, defRStages1, spdStages1,
        atkStages2, defStages2, atkRStages2, defRStages2, spdStages2
    ];

    for (let i = 0; i < stageSelects.length; i++) {
        let select = stageSelects[i];
        let val = parseInt(select.value);
        select.classList.remove("stage-pos", "stage-neg", "stage-neutral");

        if (isNaN(val)) {
            select.classList.add("stage-neutral");
        } else if (val > 0) {
            select.classList.add("stage-pos");
        } else if (val < 0) {
            select.classList.add("stage-neg");
        } else {
            select.classList.add("stage-neutral");
        }
        ...
        for (let j = 0; j < select.options.length; j++) {
            ...
            let optVal = parseInt(opt.value || opt.text);
            if (isNaN(optVal)) {
                opt.style.color = "var(--text)";
            } else if (optVal > 0) {
                opt.style.color = "var(--stage-pos)";
            } else if (optVal < 0) {
                opt.style.color = "var(--stage-neg)";
            } else {
                opt.style.color = "var(--text)";
            }
        }
...
function updateStatusHighlights() {
    let statusClasses = ["status-paralyzed", "status-burned", "status-frozen", "status-poisoned"];
    let statElements = [statHP1, statHP2, statAtk1, statAtk2, statDef1, statDef2, statAtkR1, statAtkR2, statDefR1, statDefR2, statSpd1, statSpd2];

    for (let i = 0; i < statElements.length; i++) {
        if (!statElements[i]) continue;
        statElements[i].classList.remove(...statusClasses);
    }

    applyStatusToStats(status1.value, statAtk1, statAtkR1, statSpd1);
    applyStatusToStats(status2.value, statAtk2, statAtkR2, statSpd2);
}

function applyStatusToStats(statusValue, atkElem, atkRElem, spdElem) {
    if (statusValue == "paralasis" && spdElem) {
        spdElem.classList.add("status-paralyzed");
    } else if (statusValue == "burned" && atkElem) {
        atkElem.classList.add("status-burned");
    } else if (statusValue == "freeze" && atkRElem) {
        atkRElem.classList.add("status-frozen");
    } else if (statusValue == "poisoned" || statusValue == "diseased") {
        // Poison affects output text only; no stat number highlight.
    }
}
...
function updateDiseasedColors() {
    let selects = [diseased1, diseased2];
    let start = { r: 177, g: 156, b: 222 };
    let end = { r: 110, g: 76, b: 185 };
    ...
    for (let s = 0; s < selects.length; s++) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            select.options[i].style.color = "rgb(" + r + "," + g + "," + b + ")";
            if (i === select.selectedIndex) {
                select.style.color = "rgb(" + r + "," + g + "," + b + ")";
            }
        }
    }
}
...
function loadSets(onlyFirst = false, onlySecond = false) {
    if (onlyFirst || (!onlyFirst && !onlySecond)) {
        ...
        let ivs1 = set1.ivs || {};
        hpIV1.value = ivs1.hp ?? 31;
        atkIV1.value = ivs1.attack ?? 31;
        defIV1.value = ivs1.defense ?? 31;
        atkRIV1.value = ivs1.attackR ?? 31;
        defRIV1.value = ivs1.defenseR ?? 31;
        spdIV1.value = ivs1.speed ?? 31;
    ...
    if (onlySecond || (!onlyFirst && !onlySecond)) {
        ...
        let ivs2 = set2.ivs || {};
        hpIV2.value = ivs2.hp ?? 31;
        atkIV2.value = ivs2.attack ?? 31;
        defIV2.value = ivs2.defense ?? 31;
        atkRIV2.value = ivs2.attackR ?? 31;
        defRIV2.value = ivs2.defenseR ?? 31;
        spdIV2.value = ivs2.speed ?? 31;
...
function loadStats() {
    ...
    atk1 = calculateStat(baseAtk1.value, atkIV1.value, atkEV1.value, level1.value, undefined, posNat1, "AttackM");
    def1 = calculateStat(baseDef1.value, defIV1.value, defEV1.value, level1.value, undefined, posNat1, "DefenseM");
    atkR1 = calculateStat(baseAtkR1.value, atkRIV1.value, atkREV1.value, level1.value, undefined, posNat1, "AttackR");
    defR1 = calculateStat(baseDefR1.value, defRIV1.value, defREV1.value, level1.value, undefined, posNat1, "DefenseR");
    spd1 = calculateStat(baseSpd1.value, spdIV1.value, spdEV1.value, level1.value, undefined, posNat1, "Speed");

    hp2 = calculateStat(baseHP2.value, hpIV2.value, hpEV2.value, level2.value, true, undefined, undefined, undefined);
    atk2 = calculateStat(baseAtk2.value, atkIV2.value, atkEV2.value, level2.value, undefined, posNat2, "AttackM");
    def2 = calculateStat(baseDef2.value, defIV2.value, defEV2.value, level2.value, undefined, posNat2, "DefenseM");
    atkR2 = calculateStat(baseAtkR2.value, atkRIV2.value, atkREV2.value, level2.value, undefined, posNat2, "AttackR");
    defR2 = calculateStat(baseDefR2.value, defRIV2.value, defREV2.value, level2.value, undefined, posNat2, "DefenseR");
    spd2 = calculateStat(baseSpd2.value, spdIV2.value, spdEV2.value, level2.value, undefined, posNat2, "Speed");
    ...
    let tenacity1 = (ability1 == "Tenacity" && status1.value != "healthy");
    let tenacity2 = (ability2 == "Tenacity" && status2.value != "healthy");
    ...
    if (status1.value == "freeze" && !firstLoom.types.includes("Ice") && ability1 != "Vigorous") multi *= 0.5;
    ...
    multi = 1;
	if (firstItem == "Evolutionary Transformer" && firstLoom.finalEvo == false) multi *= 1.5;
    if (tenacity1) multi *= 1.5;
    if (!tenacity1 && status1.value == "burned" && !firstLoom.types.includes("Pyro") && ability1 != "Vigorous" && ability1 != "Water Membrane") multi *= 0.5;
	trueStats1.atk = atk1;
    statAtk1.innerHTML = Math.floor(atk1 * multi);
    multi = 1;
    ...
    if (status2.value == "freeze" && !secondLoom.types.includes("Ice") && ability2 != "Vigorous") multi *= 0.5;
    ...
    multi = 1;
	if (secondItem == "Evolutionary Transformer" && secondLoom.finalEvo == false) multi *= 1.5;
    if (tenacity2) multi *= 1.5;
    if (!tenacity2 && status2.value == "burned" && !secondLoom.types.includes("Pyro") && ability2 != "Vigorous" && ability2 != "Water Membrane") multi *= 0.5;
	trueStats2.atkR = atk2;
    statAtk2.innerHTML = Math.floor(atk2 * multi);
    multi = 1;
}

function calculateStat(base, IV, EV, level, isHP = false, posNat, name, rest = false) {
    let stat;
...
function detailedReport() {
    ...
    let formatStage = function (stage) {
        if (stage > 0) return "<span class=\"stage-text-pos\">+" + stage + "</span>";
        if (stage < 0) return "<span class=\"stage-text-neg\">" + stage + "</span>";
        return "";
    };
    let colorTerrainText = function (text) {
        if (!text) return text;
        let terrainMap = [
            { label: "Flooded", cls: "terrain-flood" },
            { label: "Rocky", cls: "terrain-rocky" },
            { label: "Overgrown", cls: "terrain-overgrown" },
            { label: "Burning", cls: "terrain-burning" },
            { label: "Enchanted", cls: "terrain-enchanted" },
            { label: "Icy", cls: "terrain-icy" },
        ];
        for (let i = 0; i < terrainMap.length; i++) {
            let entry = terrainMap[i];
            let phrase = entry.label + " terrain";
            if (text.indexOf(phrase) !== -1) {
                text = text.split(phrase).join("<span class=\"" + entry.cls + "\">" + phrase + "</span>");
            }
        }
        return text;
    };
    ...
    if (atkDef.attack.stage != 0 && !isNaN(atkDef.attack.stage)) {
        tempAtk = formatStage(atkDef.attack.stage) + " ";
    }
    if (atkDef.defense.stage != 0 && !isNaN(atkDef.defense.stage)) {
        tempDef = formatStage(atkDef.defense.stage) + " ";
    }
    ...
    let statusClass = function (statusValue) {
        ...
        if (statusValue == "freeze") return "status-frozen";
        if (statusValue == "poisoned" || statusValue == "diseased") return "status-poisoned";
        return "";
    };
    let wrapStatus = function (label, statusValue) {
        if (!label) return "";
        let cls = statusClass(statusValue);
        if (!cls) return label;
        return "<span class=\"" + cls + "\">" + label + "</span>";
    };
    if (statStr) {
        statStr = wrapStatus(statStr, myStatus);
    }
    if (statStr2) {
        statStr2 = wrapStatus(statStr2, currStatus);
    }
...
if (ability1 == "Triple Threat" && (tempType == "Electric" || tempType == "Pyro" || tempType == "Hydro")) {
   ...
   if ((ability1 == "Sharp Teeth" && move.bite == true) ||
    ...
    if ((crit || (ability1 == "Brutal Wrath" && (stat2 == "poisoned" || stat2 == "diseased"))) && tempAtk.stage < 0) {
        tempAtk.atk = calculateStat(tempAtk.base, tempAtk.iv.value, tempAtk.ev.value, tempAtk.level, undefined, tempAtk.posNat, tempAtk.name);
    }
    ...
    if ((ability1 == "Hasty" && move.mr1 == "Melee Attack") ||
       ...
       (ability1 == "Tenacity" && stat1 != "healthy" && move.mr1 == "Melee Attack") ||
    ...
    if ((crit || (ability1 == "Brutal Wrath" && (stat2 == "poisoned" || stat2 == "diseased")) || move.piercing) && tempDef.stage > 0) {
        tempDef.def = calculateStat(tempDef.base, tempDef.iv.value, tempDef.ev.value, tempDef.level, undefined, tempDef.posNat, tempDef.name, tempDef.rest);
    }
    ...
    if (stat1 == "burned" && ability1 != "Tenacity" && move.mr == "Melee" && tempName != "Torment" && ability1 != "Vigorous" && ability1 != "Water Membrane" && types1.primary != "Pyro" && types1.secondary != "Pyro" && !(adaptive.mr && adaptive.mr == "Ranged")) {
        multi *= 0.5;
    }
...
function getTempAtkDef(second, mr) {
    ...
    if (mr.mr1 == "Ranged Attack") {
        if (second) {
            tempAtk = { atk: atkR2,  ev: atkREV2, iv: atkRIV2, base: baseAtkR2.value, name: "AttackR", posNat: posNat2,  stage: parseInt(atkRStages2.value), level: level2.value };
        } else {
            tempAtk = { atk: atkR1,  ev: atkREV1, iv: atkRIV1, base: baseAtkR1.value, name: "AttackR", posNat: posNat1,  stage: parseInt(atkRStages1.value), level: level1.value };
        }    
    }
    else if (mr.mr1 == "Melee Attack") {
        if (second) {
            if (mr.name == "Tricky Tactics") {
                tempAtk = { atk: atk1,  ev: atkEV1, iv: atkIV1, base: baseAtk1.value, name: "AttackM", posNat: posNat1,  stage: parseInt(atkStages1.value), level: level1.value };
            } else {
                tempAtk = { atk: atk2,  ev: atkEV2, iv: atkIV2, base: baseAtk2.value, name: "AttackM", posNat: posNat2,  stage: parseInt(atkStages2.value), level: level2.value };
            }
        } else {
            if (mr.name == "Tricky Tactics") {
                tempAtk = { atk: atk2,  ev: atkEV2, iv: atkIV2, base: baseAtk2.value, name: "AttackM", posNat: posNat2,  stage: parseInt(atkStages2.value), level: level2.value };
        ...
        if (second) {
            tempAtk = { atk: defR2,  ev: defREV2, iv: defRIV2, base: baseDefR2.value, name: "DefenseR", posNat: posNat2,  stage: parseInt(defRStages2.value), level: level2.value };
        } else {
            tempAtk = { atk: defR1, ev: defREV1, iv: defRIV1, base: baseDefR1.value, name: "DefenseR", posNat: posNat1,  stage: parseInt(defRStages1.value), level: level1.value };
        }
    }
    else if (mr.mr1 == "Melee Defense") {
        if (second) {
            tempAtk = { atk: def2,  ev: defEV2, iv: defIV2, base: baseDef2.value, name: "DefenseM", posNat: posNat2, stage: parseInt(defStages2.value), level: level2.value };
        } else {
            tempAtk = { atk: def1,  ev: defEV1, iv: defIV1, base: baseDef1.value, name: "DefenseM", posNat: posNat1,  stage: parseInt(defStages1.value), level: level1.value };
        }
    }
    else if (mr.mr1 == "Speed") {
        if (second) {
            tempAtk = { atk: spd2,  ev: spdEV2, iv: spdIV2, base: baseSpd2.value, name: "Speed", posNat: posNat2,  stage: parseInt(spdStages2.value), level: level2.value };
        } else {
            tempAtk = { atk: spd1,  ev: spdEV1, iv: spdIV1, base: baseSpd1.value, name: "Speed", posNat: posNat1,  stage: parseInt(spdStages1.value), level: level1.value };
        }
    }
    else {
        tempAtk = { atk: spd2,  ev: spdEV2, iv: spdIV2, base: baseSpd2.value, name: "Speed", posNat: posNat2,  stage: parseInt(spdStages2.value), level: level2.value };
    ...
    if (mr.mr2 == "Ranged Attack") {
        if (second) {
            tempDef = { def: atkR1,  ev: atkREV1, iv: atkRIV1, base: baseAtkR1.value, name: "AttackR", posNat: posNat1, stage: parseInt(atkRStages1.value), level: level1.value, rest: rest1 };
        } else {
            tempDef = { def: atkR2,  ev: atkREV2, iv: atkRIV2, base: baseAtkR2.value, name: "AttackR", posNat: posNat2,  stage: parseInt(atkRStages2.value), level: level2.value, rest: rest2 };
        }
    }
    else if (mr.mr2 == "Melee Attack") {
        if (second) {
            tempDef = { def: atk1,  ev: atkEV1, iv: atkIV1, base: baseAtk1.value, name: "AttackM", posNat: posNat1,  stage: parseInt(atkStages1.value), level: level1.value, rest: rest1 };
        } else {
            tempDef = { def: atk2,  ev: atkEV2, iv: atkIV2, base: baseAtk2.value, name: "AttackM", posNat: posNat2,  stage: parseInt(atkStages2.value), level: level2.value, rest: rest2 };
        }
    }
    else if (mr.mr2 == "Ranged Defense" && !(mr.name == "Rapid Rush" && trueStats2.defR > trueStats2.def)) {
        if (second) {
            tempDef = { def: defR1, ev: defREV1, iv: defRIV1, base: baseDefR1.value, name: "DefenseR", posNat: posNat1,  stage: parseInt(defRStages1.value), level: level1.value, rest: rest1 };
        } else {
            tempDef = { def: defR2,  ev: defREV2, iv: defRIV2, base: baseDefR2.value, name: "DefenseR", posNat: posNat2,  stage: parseInt(defRStages2.value), level: level2.value, rest: rest2 };
        }
    }
    else if (mr.mr2 == "Melee Defense" || (mr.name == "Rapid Rush" && trueStats2.defR > trueStats2.def)) {
        if (second) {
            tempDef = { def: def1,  ev: defEV1, iv: defIV1, base: baseDef1.value, name: "DefenseM", posNat: posNat1,   stage: parseInt(defStages1.value), level: level1.value, rest: rest1 };
        } else {
            tempDef = { def: def2,  ev: defEV2, iv: defIV2, base: baseDef2.value, name: "DefenseM", posNat: posNat2,   stage: parseInt(defStages2.value), level: level2.value, rest: rest2 };
        }
    }
    else if (mr.mr2 == "Speed") {
        if (second) {
            tempDef = { def: spd1,  ev: spdEV1, iv: spdIV1, base: baseSpd1.value, name: "Speed", posNat: posNat1, stage: parseInt(spdStages1.value), level: level1.value, rest: rest1 };
        } else {
            tempDef = { def: spd2,  ev: spdEV2, iv: spdIV2, base: baseSpd2.value, name: "Speed", posNat: posNat2,  stage: parseInt(spdStages2.value), level: level2.value, rest: rest2 };
        }
    }
    else {
        tempDef = { def: spd2,  ev: spdEV2, iv: spdIV2, base: baseSpd2.value, name: "Speed", posNat: posNat2, stage: parseInt(spdStages2.value), level: level2.value, rest: rest2 };
